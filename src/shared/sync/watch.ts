...
let watchedLeagueId: string | null = null;
...
export function watchActiveLeague(leagueId: string | null): void {
  ...
  if (!shouldCloudSync(user) || !isOnline() || !leagueId) {
    stopWatchers();
    return;
  }
  if (watchedLeagueId === leagueId && unsubscribers.length > 0) {
    return;
  }

  stopWatchers();
  watchedLeagueId = leagueId;
